...
public class ByteArrayEdgesClearable<I extends WritableComparable, E extends Writable>
    ...
    public void add(List<Edge<I, E>> edgeList) {
        ExtendedDataOutput extendedDataOutput =
                getConf().createExtendedDataOutput(
                        serializedEdges, serializedEdgesBytesUsed);
        try {
            for(Edge<I,E> e : edgeList) {
                WritableUtils.writeEdge(extendedDataOutput, e);
            }
        } catch (IOException e) {
            throw new IllegalStateException("add: Failed to write to the new " +
                    "byte array");
